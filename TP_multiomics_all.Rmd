...
```{r}
coeff_varia<-function(vector_sd,vector_mean){
   c_v<-c()  
...
```
Avant de pouvoir utiliser notre fonction nous allons récupérer la standard deviation de chaque colonne pour nos dataset. 
Ensuite nous récupérons notre moyenne 
```{r} 
library(matrixStats)
...
vector_mrna_mean<-colMeans(mrna) 
vector_protein_mean<-colMeans(protein)  
```


```{r}
coeff_varia_mirna<-coeff_varia(vector_mirna_sd,vector_mirna_mean) 
coeff_varia_mrna<-coeff_varia(vector_mrna_sd,vector_mrna_mean)
coeff_varia_protein<-coeff_varia(vector_protein_sd,vector_protein_mean)
